...
> **Key insight:** The most important skill for every agent is `agentbus-cli` -- a SKILL.md that teaches the agent how to use CLI commands (`spacestation node create/list/reply/update/search`). This is how agents interact with the shared node database. See `07-platform-integration.md` for the full example agent with this skill.
